...
// whatever format.
// Right now it relies on the template-creator which while it works I'm not sure it's as full-featured
// as we'll need for cleaning up attributes and childNodes etc.
// I've started identifying some of the useful clean-up operations we need and writing tests for them
...
exports.Exporter = Montage.create(Montage, {
        ...
        value: function (originalNode) {
            ...
            if (originalNode.component) {
               ...
               //TODO if no component should we choke if they have a data-montage-id?
                ...
                //TODO when do we preserve children of a component's element
                // Preserve child nodes
                //TODO we may not always want to do this
                for (i = 0; (iChild = originalNode.childNodes[i]); i++) {
                    node.appendChild(this.exportNode(iChild));
                }
    ...
    // TODO how should this be configured, does it depend on the node itself
    // seems like it could be contextual as to what attributes we want
    _ignoredAttributes: {
    ...
    _sanitizeAttributes: {
        value: function (node) {
            ...
            // The attributed NamedNodeList is live so you can't remove
            // while iterating through it, order is not guaranteed so
            // going backwards won't help
            for (i = 0; (iAttribute = attributes[i]); i++) {
                iAttributeName = iAttribute.name;
                if (this._ignoredAttributes.indexOf(iAttributeName) === -1) {
                    attributesToRemove.push(iAttributeName);
                }
            }

            for (i = 0; (iAttributeName = attributesToRemove[i]); i++) {
                node.removeAttribute(iAttributeName);
            }
        }
    },

    // NOTE I've included what appears to be relevant code for saving a live document being edited inside of ninja
            ...
            if (template.webgl && template.webgl.length > 1) {//TODO: Should be length 0, hack for a temp fix
                ...
                for (var i in this.application.ninja.coreIoApi.ninjaLibrary.libs) {
                    ...
                    if (this.application.ninja.coreIoApi.ninjaLibrary.libs[i].name === 'RDGE') {
                        ...
                        this.application.ninja.coreIoApi.ninjaLibrary.copyLibToCloud(template.file.root, rdgeDirName, function (result) {
                            libsobserver.canvasCopied = result;
                            this.libCopied(libsobserver);
                        }.bind(this));
                    } else {
            ...
            if (mJsSerialization) {
                ...
                for (var i in this.application.ninja.coreIoApi.ninjaLibrary.libs) {
                    ...
                    if (this.application.ninja.coreIoApi.ninjaLibrary.libs[i].name === 'Montage') {
                        ...
                        this.application.ninja.coreIoApi.ninjaLibrary.copyLibToCloud(template.file.root, mjsDirName, function (result) {
                            libsobserver.montageCopied = result;
                            this.libCopied(libsobserver);
                        }.bind(this));
